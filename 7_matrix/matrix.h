...
public:
  ...
  TwoDimensionMatrix();
  TwoDimensionMatrix(const TwoDimensionMatrix &imput_matrix);
  TwoDimensionMatrix(const MatrixElement matrix_[size][size]);

// ------------------------

  MatrixElement get(const size_t row, const size_t column) const; // const = nic nie zrobi z obiektem
  constexpr static MatrixElement getSize() { return size; }

// ------------------------

  TwoDimensionMatrix &operator=(const TwoDimensionMatrix &imput_matrix);
...
// ------------------------
  ...
  friend std::istream &operator>>(std::istream &input, TwoDimensionMatrix &input_matrix);
  friend std::ostream &operator<<(std::ostream &output, const TwoDimensionMatrix &input_matrix);
  friend TwoDimensionMatrix operator+(const TwoDimensionMatrix &matrix1, const TwoDimensionMatrix &matrix2); // jako funkcja globalna
};

// others
// std::istream &operator>>(std::istream &input, const TwoDimensionMatrix &matrix);
